...
def main(args):
    ...
    if getattr(args, 'load_transformer_body_and_src_embeddings_from') is not None:
        pretrained_state_dict = torch.load(args.load_transformer_body_and_src_embeddings_from)['model']
        for name, param in model.named_parameters():
        ...
        if args.only_finetune_cross_attn:
            ...
            for name, param in model.named_parameters():
                if 'encoder_attn' not in name:
                    ...
                    logger.info(f'parameter {name} will be trained')
            if args.also_finetune_src_embeddings:
                logger.info(f'src embeddings will also be trained, dismiss earlier info')
                for name, param in model.named_parameters():
                    if name in ['encoder.embed_tokens.weight',
                                'encoder.embed_positions.weight',
                                'encoder.layernorm_embedding.weight',
        ...
        for name, param in model.named_parameters():
            with torch.no_grad():
                ...
                if name in ['decoder.embed_tokens.weight',
                    ...
                    param.requires_grad = False
                    logger.info(f'froze parameter {name}')
    if getattr(args, 'load_franken_model_but_src_embeddings_and_freeze_tgt_embeddings_from') is not None:
        logger.info(f'loading the Frankenstein model but the src embeddings from'
                    f'{args.load_franken_model_but_src_embeddings_and_freeze_tgt_embeddings_from}')
        pretrained_state_dict = torch.load(args.load_franken_model_but_src_embeddings_and_freeze_tgt_embeddings_from)['model']
        for name, param in model.named_parameters():
            with torch.no_grad():
                if name not in ['encoder.embed_tokens.weight',
                                'encoder.embed_positions.weight',
    ...
    if getattr(args, 'load_model_but_src_embeddings_and_xattn_and_freeze_tgt_embeddings_from') is not None:
        ...
        pretrained_state_dict = torch.load(args.load_model_but_src_embeddings_and_xattn_and_freeze_tgt_embeddings_from)[
            'model']
        for name, param in model.named_parameters():
            with torch.no_grad():
                if name not in ['encoder.embed_tokens.weight',
    ...
    if getattr(args, 'finetune_from_mbart_at') is not None:
        ...
        pretrained_state_dict = model.upgrade_state_dict_named(pretrained_state_dict, '')
        model_params = set()
